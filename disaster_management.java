...
public class disaster_management {
		...
		try {
			...
			for(FogDevice device : fogDevices){
				if(device.getName().startsWith("c")){ // We later ensure that the name of each camera starts with the letter 'c'
					moduleMapping.addModuleToDevice("take-picture", device.getName());  // Since the camera takes a picture, we fix an instance of the module 'take-picture' to each CAMERA
				}
			}
			for(FogDevice device : fogDevices){
				if(device.getName().startsWith("a")){ // We later ensure that the name of each fog device starts with the letter 'a'
					moduleMapping.addModuleToDevice("free-space", device.getName());   // Since the fog node checks if free space is available, we fix an instance of the module 'free-space' to each fog device
				}
			}
			...
			if(CLOUD){
				...
				moduleMapping.addModuleToDevice("take-picture", "cloud"); // placing all instances of Object Detector module in the Cloud
				moduleMapping.addModuleToDevice("free-space", "cloud"); // placing all instances of Object Tracker module in the Cloud
			}
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		FogDevice fog_node = createFogDevice("a-"+id, 2000, 4000, 1000, 10000, 2, 0.0, 107.339, 83.4333); //ensuring fog nodes start with the letter 'a'
		...
		for(int i=0;i<numOfcamerasPerArea;i++){
			...
			FogDevice camera_sensor = add_camera_sensor(mobileId, userId, appId, fog_node.getId()); //considering the CAMERA as a fog device. (Everything on iFogSim is a fog device)
	...
	private static FogDevice add_camera_sensor(String id, int userId, String appId, int parentId){
		...
		Sensor sensor = new Sensor("s-"+id, "camera_sensor", userId, appId, new DeterministicDistribution(camera_wait)); // transmission time of CAMERA: deterministic distribution
		sensors.add(sensor);
		Actuator ptz = new Actuator("ptz-"+id, userId, appId, "PTZ_CONTROL"); //pan-tilt-zoom. Part of the CAMERA's function, and hence considered as an actuator
		actuators.add(ptz);
	...
	private static Application createApplication(String appId, int userId){
		...
		application.addAppEdge("camera_sensor", "take-picture", 1000, 500, "camera_sensor", Tuple.UP, AppEdge.SENSOR); // adding edge from camera_sensor to take_picture module 
